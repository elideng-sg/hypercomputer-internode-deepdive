...
| `NCCL_NET_PLUGIN` | Select/disable the external net plugin (`libnccl-net.so`): `none`, or a named plugin (e.g. `gpudirecttcpx`). | Confirming whether the GPUDirect-TCPX/Fast-Socket plugin is loaded; forcing the built-in socket transport for a baseline. | lab-05, lab-18 |
...
| `NCCL_GPUDIRECTTCPX_SOCKET_IFNAME` / `..._CTRL_DEV` | (TCPX plugin) the four GPU-NIC data rails and the control NIC for GPUDirect-TCPX. | Only on a multi-network TCPX-provisioned pool; set by the workload manifest, not by hand. | lab-18 |
...
| `NCCL_CROSS_NIC` | Allow a ring to cross between NICs/rails (0/1/2). | Multi-rail fabrics (TCPX/RDMA) — controls rail alignment of the ring. | lab-18 |
