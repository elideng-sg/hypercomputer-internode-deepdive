...
**Contrast with GCP A3:** The GCP compute fabric for A3 High/Mega uses **GPUDirect-TCPX/TCPXO** (a GCP NCCL plugin pairing with the `tcpgpudmarxd` receive datapath and the **Titanium** offload; discussed in Part II). Two things are true at once here, and the guide originally conflated them (logged as **D2** in [lab-build-gotchas.md](../../reference/lab-build-gotchas.md)):

- **Per-node rails *are* tenant-visible and tenant-configured.** An `a3-highgpu-8g` Pod built for TCPX has **4 dedicated GPU NICs** (`eth1–eth4`), each on its own VPC/subnet at jumbo MTU 8244, and **NCCL enumerates all four and stripes across them** — verified live in [lab-18](../../labs/lab-18-enable-gpudirect-tcpx/), where the four rails `192.168.{0,1,2,3}` each appeared **264 times** in the NCCL connect log and each carried **~50.4 GB**, balanced to within **0.05%**. The rail list is not merely visible, it is *mandatory tenant input*: `NCCL_GPUDIRECTTCPX_SOCKET_IFNAME=eth1,eth2,eth3,eth4` is written by hand (TCPX ships no env-profile script to generate it). A3 Mega/TCPXO is the same picture with **8** rails.
- **The *underlay* rail topology is opaque.** What the tenant cannot see or influence is which leaf/spine each rail lands on, whether rails are non-overlapping trees end-to-end, or how Jupiter routes them. There is no equivalent of a DGX rail map, and a hand-authored NCCL topology file cannot express switch-level rail affinity that Google does not expose.

So the accurate contrast is **not** "one pipe vs eight rails" — it is **tenant-visible NIC-level rails over a cloud-managed, opaque underlay** versus SuperPOD's **tenant-known rail-to-switch topology**. A3 Ultra/A4 (ConnectX-7 RoCE) moves closer still to the SuperPOD model — real RDMA verbs, `NET/IB`, `ibv_devinfo`-discoverable devices — but the switch fabric remains Google-managed either way.
...
**Contrast with GCP A3:** A3 High/Mega **does** expose multiple rails to NCCL — but only at the **NIC** level, not the **switch** level, and only if the cluster was built for it.

*What matches the rail model:* with GPUDirect-TCPX enabled, NCCL sees **4 discrete GPU NICs** on an `a3-highgpu-8g` (8 on `a3-megagpu-8g`), one VPC each, and stripes the collective across them — measured in [lab-18](../../labs/lab-18-enable-gpudirect-tcpx/): 4 rails, 264 NCCL references each, ~50.4 GB each, imbalance <0.05%, giving **83.27 GB/s busbw** on 16 GPUs versus **23.70 GB/s** on the single-`eth0` path. That 3.5× *is* the rail striping doing its job. It is also why rail **imbalance** is a real GCP failure mode with a real check ([doc-25 §4.7](../part5-operations-diagnostics/25-fabric-diagnostics-playbook.md)) — a meaningless concept if there were only one rail.

*What does not match:* the rails are **not tenant-mapped to switches**. You cannot supply an NCCL topology file describing rail-to-leaf affinity, cannot verify the rails are non-overlapping trees, and cannot pin a job to a rail — Jupiter's topology is fixed and opaque, and striping *below* the NIC is Titanium's business. On a DGX SuperPOD the rail map is a published property of the SU; on GCP it is an implementation detail.

*A caveat that follows from the underlay being invisible:* both GCP figures above are **untuned floors**. Neither run was NUMA-pinned to its rails, so the SuperPOD comparison should be read as architecture-vs-architecture, not as a bandwidth certification.

A3 Ultra/A4 (ConnectX-7 RoCE) is closer again — RDMA verbs, `NET/IB`, `ibv_devinfo` discovery — but the fabric is still **cloud-managed**: you don't configure switch topology or VLAN mappings. The **result is comparable** (published A3 Ultra/A4 RoCE all-reduce bandwidth lands within 10–20% of DGX H100 InfiniBand for large messages — vendor figures, not measured here), while the **operational model differs** (cloud-elastic vs fixed SU capacity).
...
| **GPU Fabric (Inter-Node)** | **Quantum InfiniBand** (rail-optimized HDR/NDR; 400–800 Gbps per NIC; 8 rails; SHARP in-network all-reduce) or **Spectrum-X Ethernet** (RoCE; 400 Gbps ConnectX-7 or BlueField-3 SuperNIC; adaptive routing) | **GPUDirect-TCPX** — 4 dedicated GPU NICs (`eth1–4`, jumbo MTU 8244) + `eth0` control; rxdm/Titanium offload; NCCL plugin. **NIC-level rails ARE tenant-visible and NCCL-striped** (measured: 4 balanced rails, **83.27 GB/s** busbw @ 16 GPU); the **underlay** rail-to-switch topology is not exposed |
| **Network Topology** | Spine-leaf with scalable units (SU); each SU = 32 nodes + leaf switches; multiple SUs connected via spine; rail-optimized (8 rails per node; each GPU NIC on a separate rail) | Jupiter fabric (Google Clos network; opaque topology; dynamic routing). Rails are visible to NCCL **as NICs** and must be listed by the tenant, but their **switch-level** rail-optimization is not exposed or verifiable — no NCCL topology file can express it |
