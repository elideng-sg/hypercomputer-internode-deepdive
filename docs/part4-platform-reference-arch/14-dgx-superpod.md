...
**Contrast with GCP A3:** The GCP compute fabric for A3 High/Mega uses **GPUDirect-TCPX/TCPXO** (a GCP NCCL plugin pairing with the `tcpgpudmarxd` receive datapath and the **Titanium** offload; discussed in Part II). Two things are true at once here, and the guide originally conflated them:
