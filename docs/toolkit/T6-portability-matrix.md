...
| **Inter-Node Networking — RDMA/GPUDirect** | A3 High/Mega: **GPUDirect-TCPX** (4 rails, payload still over host TCP — netdev counters *see* it) / **TCPXO/FasTrak** (8 rails, userspace datapath — netdev counters are *blind*); both via rxdm offload to GCP Titanium + an NCCL plugin `libnccl-net.so`. A3 Ultra/A4: **GPUDirect-RDMA over RoCE** (ConnectX-7). NCCL Fast Socket is a *separate, older* TCP optimisation — not GPUDirect | **GPUDirect-RDMA over InfiniBand** or **Spectrum-X RoCE Ethernet** (ConnectX/BlueField NICs); NCCL auto-detects via `NCCL_IB_*` / `NCCL_NET` (often no plugin needed for native IB; SHARP plugin for in-network reduction on Quantum IB) | GPUDirect-RDMA (if InfiniBand/RoCE NICs + MLNX_OFED / MOFED drivers installed); NCCL sees via `NCCL_IB_HCA`; verify with `ibv_devinfo`, `perftest` (`ib_write_bw`) | GPUDirect-RDMA over InfiniBand (standard HPC fabric); MOFED drivers; NCCL `NCCL_IB_*` tuning; site may provide NCCL topology files | Cloud-specific GPUDirect (AWS EFA uses `libnccl-plugin-efa.so`, Azure uses native RDMA, GCP uses TCPX/TCPXO plugins or RoCE) |
...
|:---|:---|:---|:---:|:---|:---|
| **A3 High** | `a3-highgpu-8g` | H100 80GB | 8 | **GPUDirect-TCPX** (4 GPU NICs @ MTU 8244 + `eth0` control; rxdm `tcpgpudmarxd` offload to Titanium; NCCL plugin `libnccl-net.so` into `/usr/local/nvidia/lib64`, image `gpudirect-tcpx/nccl-plugin-gpudirecttcpx-dev` — pin **`:v3.1.12`**, never `:latest`; **no** env-profile script ships, so the rail list is hand-written) | **Live lab, MEASURED** (Parts I–III on the single-gVNIC path; **lab-18** — 4 rails, `GPUDirectTCPX_v7`, **83.27 GB/s busbw @ 16 GPU** = **≈3.5×** the single-gVNIC path; Part IV contrasts with DGX) |
...
**Measured tier figures for cross-reference** (same benchmark, same GPU model, 16 GPUs / 2 nodes):
single-gVNIC `NET/Socket` **23.70 GB/s** → TCPX, 4 rails **83.27 GB/s** (**3.5×**) → TCPXO, 8 rails
**317.84 GB/s** (**13.4×**). The ladder tracks **rail count** more than tier names. Both GPUDirect
figures are **untuned floors** (no NUMA/rail pinning), and both required a purpose-built cluster —
Dataplane V2 + multi-networking are create-time-only.
